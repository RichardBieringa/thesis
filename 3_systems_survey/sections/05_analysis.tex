...
% The remainder of this section is structured as follows. In \cref{sec:survey:analysis:sm-framework}, we provide a qualitative comparison of the service mesh systems and present several key findings from the obtained data. Thereafter, in \cref{sec:survey:analysis:architectures}, we dive into the different architectures and how they can relate to the observed behaviour.
...
\begin{figure}[p]
    \centering
    \includegraphics[width=0.65\textwidth]{3_systems_survey/figures/sm-per-service-small.pdf}
    \caption[Per-service proxy data plane architecture.]{The data path of a network packet  in a per-service data plane.}
    \label{fig:sm-arch-per-service}
    
    \vspace*{0.5cm}
    
    \includegraphics[width=0.65\textwidth]{3_systems_survey/figures/sm-per-node-small.pdf}
    \caption[Per-node proxy data plane architecture.]{The data path of a network packet  in a per-node data plane.}
    \label{fig:sm-arch-per-node}
    
    \vspace*{0.5cm}
   
    \includegraphics[width=0.65\textwidth]{3_systems_survey/figures/sm-ebpf-small.pdf}  
    \caption[\Gls{ebpf}-based data plane architecture.]{The data path of a network packet in an \textit{\gls{ebpf}-based} data plane.}
